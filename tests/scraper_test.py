...
class TestGenerateEventDates:
    ...
    def test_generate_event_dates_should_raise_exception_if_start_date_greater_end_date(
        self,
    ):
        # Given
        scraper = KLPScraper()
...
class TestParseEventRow:
    ...
    def test_extract_time_should_return_none_if_no_time(self):
        ...
        scraper = KLPScraper()
        time_div = BeautifulSoup(
            '<div><b>29.05.</b> | <a href="/radrouten.html#t4" target="_radrouten">Fahrradtour:<span class="num">4</span></a></div>',
            features="html.parser",
        )
    ...
    def test_extract_location_should_return_none_none_if_no_link(self):
        ...
        scraper = KLPScraper()
        location_div = BeautifulSoup(
            "<div> (Meike Klapprodt)</div>", features="html.parser"
        )
    ...
    def test_extract_location_should_return_none_for_slug_if_no_matching_slug(self):
        ...
        scraper = KLPScraper()
        location_div = BeautifulSoup(
            '<div><a href="/orte/" title="Wunderpunkt anzeigen">MIDDEFEITZ</a> (Meike Klapprodt)</div>',
            features="html.parser",
        )
    ...
    def test_extract_location_should_return_correct_event_name_and_description(self):
        # Given
        scraper = KLPScraper()
        event_description_div = BeautifulSoup(
            "<div><b>Bewegen am Morgen</b><br>Wir starten bewegt in den Tag. <br>Eintritt: Hutkasse</div>",
            features="html.parser",
        )
    ...
    def test_extract_location_should_return_default_title_if_title_is_missing(self):
        ...
        scraper = KLPScraper()
        event_description_div = BeautifulSoup(
            "<div><br>Wir starten bewegt in den Tag. <br>Eintritt: Hutkasse</div>",
            features="html.parser",
        )
