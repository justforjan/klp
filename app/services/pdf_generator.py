...
def generate_favorites_pdf(events_data: dict, exhibitions_data: dict = None) -> BytesIO:
    ...
    story.append(
        Paragraph(
            f"Exportiert am: {datetime.now().strftime('%d.%m.%Y um %H:%M Uhr')}",
            normal_style,
        )
    )
        ...
        for location_data in exhibitions_data.values():
            ...
            for exhibition in exhibitions:
                ...
                if exhibition.get("description"):
                    story.append(
                        Paragraph(f"  <i>{exhibition['description']}</i>", normal_style)
                    )
                story.append(Spacer(1, 6))
    ...
    if events_data and len(events_data) > 0:
        ...
        for date_str in sorted_dates:
            ...
            for item in day_events:
                ...
                story.append(
                    Paragraph(
                        f"<b>{start_time} Uhr - {event_name}</b>", event_title_style
                    )
                )
                ...
                elif event["payment_type"] == "fixed_price" and event.get(
                    "entry_price"
                ):
                ...
                if payment_info:
                    story.append(
                        Paragraph(
                            f"<b>Eintritt:</b> {' | '.join(payment_info)}", normal_style
                        )
                    )
                ...
                if event.get("organizer"):
                    story.append(
                        Paragraph(
                            f"<b>Veranstalter:</b> {event['organizer']}", normal_style
                        )
                    )
