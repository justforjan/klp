...
class KLPScraper(BaseScraper):
    ...
    async def scrape_events_for_date(self, client: httpx.AsyncClient, date_str: str) -> list[dict]:
    ...
    def parse_event_row(self, row, date_str: str) -> Optional[dict]:
        ...
        payment_type, entry_price, material_cost = self.extract_payment_info(description)
    ...
    def extract_payment_info(self, description: str) -> tuple[str, Optional[Decimal], Optional[Decimal]]:
                    ...
                    if details:
                        ...
                        if 'image_url' in details:
                            ...
                            if expected_filepath.exists():
                                ...
                                image_path = await self.download_location_image(client, details['image_url'], location_slug)
                                if image_path:
                        ...
                        if 'exhibitions' in details and details['exhibitions']:
                            await self.scrape_and_save_exhibitions(client, location, details['exhibitions'], session)

                except Exception as e:
                    print(f"Error scraping location details for {location.name}: {e}")
                    session.rollback()

    async def scrape_and_save_exhibitions(self, client: httpx.AsyncClient, location: Location, exhibitions_data: list[dict], session: Session) -> None:
    ...
    async def download_location_image(self, client: httpx.AsyncClient, image_url: str, location_slug: str) -> Optional[str]:
        try:
            if not image_url.startswith('http'):
                image_url = f"{BASE_URL}{image_url}"
    ...
    async def scrape_location_details(self, client: httpx.AsyncClient, location: Location) -> dict:
        try:
