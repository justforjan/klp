...
class KLPScraper(BaseScraper):
    async def run_initial_import(self) -> None:
        ...
        async with httpx.AsyncClient() as client:
            ...
            dates = self.generate_event_dates(start_date, end_date)

            for date_str in dates:
                print(f"Scraping events for {date_str}...")
                try:
                    events = await self.scrape_events_for_date(client, date_str)
            ...
            self.batch_insert_events(all_events_data)

            print("Starting location details scraping...")
            await self.scrape_all_location_details(client)

        print("Data import completed")

    def generate_event_dates(self, start: date, end: date) -> list[str]:
        if start > end:
    ...
    async def scrape_events_for_date(
        ...
        for row in event_rows:
            try:
                event_data = self.parse_event_row(row, date_str)
                if event_data:
    ...
    def parse_event_row(self, row, date_str: str) -> Optional[dict]:
        ...
        time_str = self.extract_time(time_div)
        if not time_str:
            return None

        location_name, location_slug = self.extract_location(location_div)
        if not location_name or not location_slug:
            return None

        event_name, description = self.extract_event_details(content_div)
        if not event_name:
            return None

        payment_type, entry_price, material_cost = self.extract_payment_info(
            description
        )
        booking_required = (
            "anmeld" in description.lower() or "buchung" in description.lower()
        )
        organizer = self.extract_organizer(location_div)
        cancellation_phrases = ["fällt aus", "fällt leider aus", "fällt weg"]
        cancelled = any(
            phrase in description.lower() for phrase in cancellation_phrases
        )
        if not cancelled:
            cancelled = any(
                phrase in event_name.lower() for phrase in cancellation_phrases
            )
    ...
    def extract_time(self, div) -> Optional[str]:
        nobr = div.find("nobr")
        if nobr:
    ...
    def extract_location(self, div) -> tuple[Optional[str], Optional[str]]:
    ...
    def extract_event_details(self, div) -> tuple[Optional[str], str]:
        bold = div.find("b")
    ...
    def extract_organizer(self, div) -> Optional[str]:
        text = div.get_text(strip=True)
        ...
        with get_session_ctx() as session:
            ...
            for event_data in events_data:
                ...
                if location_name not in location_map:
                    original_page_url = (
                        f"{BASE_URL}/orte/{location_slug}.html"
                        if location_slug
                        else None
                    )
                    location = Location(
            ...
            print(
                f"Inserted {len(locations)} locations, {len(events)} events, {len(occurrences)} occurrences"
            )

    async def scrape_all_location_details(self, client: httpx.AsyncClient) -> None:
        ...
        with get_session_ctx() as session:
            ...
            for location in locations:
                ...
                try:
                    details = await self.scrape_location_details(client, location)
                    if details:
                        ...
                        if "image_url" in details:
                            location_slug = (
                                location.original_page_url.split("/")[-1].replace(
                                    ".html", ""
                                )
                                if location.original_page_url
                                else f"location_{location.id}"
                            )
                            ...
                            if expected_filepath.exists():
                                print(
                                    f"Image already exists for {location.name} at {expected_filepath}, skipping download"
                                )
                                if not location.image_path:
                                    location.image_path = (
                                        f"/static/locations/{expected_filename}"
                                    )
                                    session.add(location)
                                    session.commit()
                            else:
                                print(f"Downloading image for {location.name}...")
                                image_path = await self.download_location_image(
                                    client, details["image_url"], location_slug
                                )
                                if image_path:
                                    ...
                                    print(
                                        f"Failed to download image for {location.name}"
                                    )

                        if "exhibitions" in details and details["exhibitions"]:
                            await self.scrape_and_save_exhibitions(
                                client, location, details["exhibitions"], session
                            )
    ...
    async def scrape_and_save_exhibitions(
        self,
        client: httpx.AsyncClient,
        location: Location,
        exhibitions_data: list[dict],
        session: Session,
    ) -> None:
        ...
        for exhibition_data in exhibitions_data:
            try:
                ...
                if existing_exhibition:
                    print(
                        f"  Exhibition '{exhibition_data['name']}' by {exhibition_data['artist']} already exists, skipping"
                    )
                    continue
                ...
                if "image_url" in exhibition_data:
                    ...
                    safe_artist_name = (
                        re.sub(r"[^\w\s-]", "", exhibition_data["artist"])
                        .strip()
                        .replace(" ", "_")
                    )
                    safe_exhibition_name = (
                        re.sub(r"[^\w\s-]", "", exhibition_data["name"])
                        .strip()
                        .replace(" ", "_")
                    )
                    ...
                    if image_filepath.exists():
                        print(
                            f"  Exhibition image already exists at {image_filepath}, skipping download"
                        )
                        exhibition.image_path = f"/static/exhibitions/{image_filename}"
                    else:
                        print(
                            f"  Downloading image for exhibition '{exhibition_data['name']}'..."
                        )
                        try:
                            ...
                            exhibition.image_path = (
                                f"/static/exhibitions/{image_filename}"
                            )
                ...
                session.commit()
                print(
                    f"  Saved exhibition '{exhibition_data['name']}' by {exhibition_data['artist']}"
                )
    ...
    async def download_location_image(
        self, client: httpx.AsyncClient, image_url: str, location_slug: str
    ) -> Optional[str]:
        try:
            if not image_url.startswith("http"):
                image_url = f"{BASE_URL}{image_url}"
    ...
    async def scrape_location_details(
            ...
            if i > 0 and "<br" in str(p).lower():
                ...
                if len(br_parts) >= 2:
                    for part in br_parts:
                        if "&" not in part and not any(
                            x in part.lower() for x in ["fon", "mail", "e.v.", "gbr"]
                        ):
        ...
        exhibitions = self.scrape_exhibitions(soup)
        if exhibitions:
            data["exhibitions"] = exhibitions

        return data

    def scrape_exhibitions(self, soup: BeautifulSoup) -> list[dict]:
        exhibitions = []

        slider_div = soup.find(
            "div", class_=lambda x: x and "slider" in x and "aus" in x
        )
        if not slider_div:
            return exhibitions
